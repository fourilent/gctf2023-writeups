"""
Source: https://github.com/MosheWagner/PyChefInterpreter
"""
import copy
import datetime
import random
...
class Chef:
    ...
    def parse(self):
        ...
        self.comment = re.match(r"(.*?)\n\n", self.script, re.DOTALL)
        # Find a comment, and replace it.
        if (
            self.comment != None
            and re.match(r"^Ingredients", self.comment.group()) == None
        ):
        ...
        self.ingr = re.match(r"Ingredients\.\n", self.script)
    ...
    def ambigcheck(self, text, dish=False):
        ...
        if re.match(r"the (1st|2nd|3rd|[0-9]+th) mixing bowl", text) != None:
            print("Ambigious mixing bowl")
            sys.exit(-1)
        if dish == True:
            if re.match(r"the (1st|2nd|3rd|[0-9]+th) baking dish", text) != None:
    ...
    def valuecheck(self, ingredient):
        ...
        if self.ingredientlist[ingredient][0] == None:
            print(
                "Cooking time error: tried to access ingredient",
    ...
    def stir(self, ingredient, mixingbowl):
        ...
        self.mixingbowls[key] = (
            self.mixingbowls[key][0 : -value - 1]
            + [self.mixingbowls[key][-1]]
            + self.mixingbowls[key][-value - 1 : -1]
        )
    ...
    def execute(self, text, loop=False):
        ...
        for ex in excode:
            ...
            put = re.search(
                r"^Put ?([a-zA-Z ]+) into (?:the )?(?:([1-9]\d*)(?:st|nd|rd|th) )?mixing bowl",
                ex,
            )
            if put != None:
                if put.group(2) == None:
            ...
            fold = re.search(
                r"Fold ([a-zA-Z ]+) into (?:the )?(?:(1st|2nd|3rd|[0-9]+th) )?mixing bowl",
                ex,
            )
            if fold != None:
            ...
            add = re.search(
                r"Add ([a-zA-Z ]+?) to (?:the )?(?:(1st|2nd|3rd|[0-9]+th) )?mixing bowl",
                ex,
            )
            if add != None:
                if add.group(2) == None:
            ...
            remove = re.search(
                r"Remove ([a-zA-Z ]+?) from (?:the )?(?:(1st|2nd|3rd|[0-9]+th) )?mixing bowl",
                ex,
            )
            if remove != None:
            ...
            combine = re.search(
                r"Combine ([a-zA-Z ]+?) into (?:the )?(?:(1st|2nd|3rd|[0-9]+th) )?mixing bowl",
                ex,
            )
            if combine != None:
            ...
            stir = re.search(
                r"Stir ([a-zA-Z ]+?) into (?:the )?(?:(1st|2nd|3rd|[0-9]+th) )?mixing bowl",
                ex,
            )
            if stir != None:
            ...
            divide = re.search(
                r"Divide ([a-zA-Z ]+?) to (?:the )?(?:(1st|2nd|3rd|[0-9]+th) )?mixing bowl",
                ex,
            )
            if divide != None:
            ...
            liquefy = re.search(
                r"Liquefy contents of the (1st|2nd|3rd|[0-9]+th)? ?mixing bowl", ex
            )
            if liquefy != None:
                if liquefy.group(1) == None:
                    ...
                    for i in self.mixingbowls[0]:
                        if i[1] == "dry":
                            i[1] = "liquid"
                continue
            liquefy2 = re.search(r"Liquefy [a-zA-Z]", ex)
            if liquefy != None:
                self.ingredientlist[liquefy.group(1)]
                continue
            clean = re.search(r"Clean the (1st|2nd|3rd|[0-9]+th)? ?mixing bowl", ex)
            if clean != None:
                if clean.group(1) == None:
                    ...
                    if clean.group(1)[:-2] in self.mixingbowls:
                        ...
                        if existslater == None:
                            ...
                            print(
                                "Warning: Tried to clean mixingbowl",
                                clean.group(1),
                                "before putting anything in it!",
                            )
                continue
            mix = re.search(r"Mix the (1st|2nd|3rd|[0-9]+th)? ?mixing bowl well", ex)
            if mix != None:
                if mix.group(1) == None:
                    ...
                    if mix.group(1)[:-2] in self.mixingbowls:
                        ...
                        if existslater == None:
                            ...
                            print(
                                "Warning: Tried to mix mixing bowl",
                                mix.group(1),
                                "before putting anything in it!",
                            )
                continue
            fridge = re.search(r"Take ([a-zA-Z]+) from refrigerator", ex)
            if fridge != None:
                ...
                continue
            pour = re.search(
                r"Pour contents of the (?:the )?(?:([1-9]\d*)(?:st|nd|rd|th) )?mixing bowl into the (?:the )?(?:([1-9]\d*)(?:st|nd|rd|th) )?baking dish",
                ex,
            )
            if pour != None:
                ...
                continue
            refer = re.search(r"Refrigerate (?:for ([0-9]+))? hours", ex)
            if refer != None:
                if refer.group(1) != None:
                    self.serve(refer.group(1))
                sys.exit()
            adddry = re.search(
                r"Add dry ingredients(?: to the (1st|2nd|3rd|[0-9]+th) mixing bowl)?",
                ex,
            )
            if adddry != None:
            ...
            auxiliary = re.match(r"Serve with ([a-zA-Z ]+)", ex)
            if auxiliary != None:
            ...
            verb = re.search(r"([a-zA-Z]+) the ([a-zA-Z ]+) ?(?!until)", ex)
            if verb != None:
                ...
                if self.ingredientlist[verb.group(2)][0] == 0:
                    ...
                    looptext = re.search(
                        verb.group()
                        + r"\.((.*?)\s+[a-zA-Z]+ (?:the ([a-zA-Z ]+)) until "
                        + verbw
                        + "ed)",
                        text,
                        re.DOTALL | re.IGNORECASE,
                    )
                    deltext = re.split(r"\.\s+", looptext.group(1))
            ...
            if loop == True:
                setaside = re.search(r"Set aside", ex)
                if setaside != None:
                    return "ENDOFLOOP"
